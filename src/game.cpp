	...
	void Game::addMoney(Cylinder* cylinder, uint64_t money, uint32_t flags /*= 0*/)
{
    ...
    uint64_t goldBars = money / 10000000000ULL;
    money -= goldBars * 10000000000ULL;
    while (goldBars > 0) {
    ...
    uint64_t goldNuggets = money / 100000000;
    if (goldNuggets != 0) {
        Item* remaindItem = Item::CreateItem(ITEM_GOLD_NUGGET, goldNuggets);
        ReturnValue ret = internalAddItem(cylinder, remaindItem, INDEX_WHEREEVER, flags);
        if (ret != RETURNVALUE_NOERROR) {
            internalAddItem(cylinder->getTile(), remaindItem, INDEX_WHEREEVER, FLAG_NOLIMIT);
        }
	...
	if (damage.primary.value > 0) {
		...
		if (attackerPlayer) {
			...
			if (chance != 0 && uniform_random(1, 100) <= chance) {
				...
				lifeLeech.primary.value = std::round(healthChange * (attackerPlayer->getSkillLevel(SKILL_LIFE_LEECH_AMOUNT) / 100.));
			...
			if (chance != 0 && uniform_random(1, 100) <= chance) {
				...
				manaLeech.primary.value = std::round(healthChange * (attackerPlayer->getSkillLevel(SKILL_MANA_LEECH_AMOUNT) / 100.));
				g_game.combatChangeMana(nullptr, attackerPlayer, manaLeech);
			}

			chance = attackerPlayer->getSkillLevel(SKILL_CRITICAL_HIT_CHANCE);
			if (chance != 0 && uniform_random(1, 100) <= chance) {
				healthChange += std::round(healthChange * (attackerPlayer->getSkillLevel(SKILL_CRITICAL_HIT_DAMAGE) / 100.));
...
void Game::checkLight()
{
	...
	if (newLightLevel <= LIGHT_LEVEL_DAY) {
