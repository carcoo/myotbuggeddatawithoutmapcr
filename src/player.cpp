...
bool Player::canWalkthrough(const Creature* creature) const
{
	...
	if ((OTSYS_TIME() - lastWalkthroughAttempt) > 1) {
...
void Player::death(Creature* lastHitCreature)
{
	...
	if (skillLoss) {
		...
		//Level loss
		uint64_t expLoss = static_cast<uint64_t>(experience * 0.001);
...
bool Player::updateSaleShopList(const Item* item)
{
    ...
    if (itemId != ITEM_GOLD_COIN && itemId != ITEM_PLATINUM_COIN && itemId != ITEM_CRYSTAL_COIN && itemId != ITEM_SCARAB_COIN && itemId != ITEM_GOLD_NUGGET && itemId != ITEM_GOLD_BAR) {
...
{
	if ((OTSYS_TIME() - lastToggleMount) < 100 && !wasMounted) {
