...
function onSay(cid, words, param, channel)
        ...
        if(t[1] == "add") then
                ...
                local check = db.storeQuery("SELECT `id` FROM `auction_system` WHERE `player` = " .. getPlayerGUID(cid) .. ";")
        ...
        if(t[1] == "withdraw") then
                local balance = db.storeQuery("SELECT `auction_balance` FROM `players` WHERE `id` = " .. getPlayerGUID(cid) .. ";")
