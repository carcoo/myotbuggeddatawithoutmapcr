...
local check = db.storeQuery("SELECT `frags` FROM `guilds` WHERE `id` = " ..getGuildId(GuildName))
...
function getAcessTime(GuildName)
		local query = db.storeQuery("SELECT `acesstime` FROM `guilds` WHERE `id` = " ..getGuildId(GuildName))
...
local PlayersGuild = db.storeQuery("SELECT `name` FROM `players` WHERE `rank_id` IN (SELECT `id` FROM `guild_ranks` WHERE `guild_id` = " .. getGuildId(GuildName) .. ");")  
...
local getHonor = db.storeQuery("SELECT `value` FROM `player_storage` WHERE `player_id` = ".. Guid .." AND `key` = ".. frag_guild.Honor_Storage)
